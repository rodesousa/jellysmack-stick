...
// Saves grid ids
type Pipes = Vec<(usize, usize)>;

// Reads the file and return head/tail pair and pipes
pub fn engine() -> Result<(Line, Line, Pipes), StickError> {
    ...
    match io::stdin().read_line(&mut input_line) {
        ...
        Ok(_) => {
            ...
            let (head, tail, result) = find_pipes(width, height)?;
            Ok((head, tail, result))
        }
    }
}

// Swaps tail to find pairs
pub fn solution(head: Line, mut tail: Line, result: Pipes) -> (Line, Line) {
...
fn find_pipes(width: i32, height: i32) -> Result<(Line, Line, Pipes), StickError> {
    let mut result: Pipes = Vec::new();
